...
#if defined(NDEBUG)
# error "Trihardcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc5c9f3c926a176d2069cfd1731238b2a0375cb7b8cce50d2c44a9117bb2ec230");
...
const string strMessageMagic = "Trihardcoin Signed Message:\n";
...
{
    // Trihardcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Trihardcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Trihardcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Trihardcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Trihardcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Trihardcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x772d7f3d38e2d33effe3f82dbcc09669effaef80b25ea1c53ff7dcc507d58d6e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Actor Vivek passes away aged 59 from Cardiac Arrest";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044a77ccfd344e88397330efa28eb1a837b5b0c4a5bc0071da53a4e2de4434bb6289fa30b141db1573c20c4000c3bfaad386df2c0423ff8b50716ba7dc4c353279") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1618709379;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085301983;

        if (fTestNet)
        {
            block.nTime    = 1618709364;
            block.nNonce   = 385299889;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x07e499db67ea4e6c707852792b62cc3e6a6040198451eaf21c94fd034ad0ad6c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc1, 0xbc, 0xd3 }; // Trihardcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TrihardcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TrihardcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TrihardcoinMiner(CWallet *pwallet)
{
    printf("TrihardcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("trihardcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TrihardcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TrihardcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TrihardcoinMiner, pwallet));
}
